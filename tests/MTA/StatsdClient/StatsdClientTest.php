...
use MTA\StatsdClient\Entity\StatsdData;
...
class StatsdClientTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function mockSenderWithAssertionOnWrite($messageToAssert=null) {
        ...
        $mock = $this->getMockBuilder('\MTA\StatsdClient\Sender\SocketSender') ->disableOriginalConstructor() ->getMock();
    ...
    public function mockFactory() {
        ...
        $mock =  $this->getMock('\MTA\StatsdClient\Factory\StatsdDataFactory', array('timing'));
    ...
    {
        $senderMock = $this->getMock('MTA\StatsdClient\Sender\SenderInterface');
        $senderMock
            ->expects($this->once())
            ->method('open')
            ->will($this->returnValue(true))
